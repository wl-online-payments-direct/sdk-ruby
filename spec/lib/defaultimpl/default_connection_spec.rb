...
describe DefaultConnection do
  ...
  body:         "(.*)"}
        ...
        let(:regex) { Regexp.new("#{request_regex}\n#{response_regex}") }

        let(:with_body_regex) do
          Regexp.new("#{request_with_body_regex}\n#{response_regex}")
        end
  ...
  context 'connection' do
      ...
      server = Thread.new do
        class DummyServer < Sinatra::Application
          ...
          get '/convert_amount.json' do
            content_type :json
            IO.read("#{prefix}testConnection.json")
          end
    ...
    it 'retrieves the correct json file' do
      ...
      prefix = 'spec/fixtures/resources/defaultimpl/'
      expect(response_body).to eq(IO.read("#{prefix}testConnection.json"))
    end
